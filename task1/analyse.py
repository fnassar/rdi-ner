...
with io.open("ANERCorp_Benajiba.txt", 'r', encoding='utf-8') as file1:
    ...
    for line in file1:
        ...
        # analyze line
        if ((len(line2[0]) == 1)):
            if (not ((ord(line2[0]) < 0x064A and ord(line2[0]) > 0x0620) or (ord(line2[0]) > 0x30 and ord(line2[0]) < 0x39))):
                continue
        # count word details
        wordList.append([line2[0], line2[1], len(line)-2], len(line2[0]))
        ...
        if line2[0] == '.':
            ...
            sentenceList.append([sentence, numentities, numWords, numChars, types])
            sentence = ""
            numentities = 0
            numChars = 0
            numWords = 0
